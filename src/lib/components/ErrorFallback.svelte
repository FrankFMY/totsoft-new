...
<div class="min-h-screen bg-dark-900 flex items-center justify-center px-4">
	<div
		class="max-w-md w-full bg-dark-800 border border-red-500/30 rounded-2xl p-8 text-center"
	>
		<div
		...
		<p class="text-slate-400 mb-6">
			Произошла непредвиденная ошибка. Пожалуйста, обновите страницу или
			попробуйте позже.
		</p>
		{#if dev && error}
			<details class="mb-6 text-left">
				...
				<pre
					class="text-xs text-red-400 bg-dark-900 p-4 rounded-lg overflow-auto">
          {error.toString()}
          {error.stack}
        </pre>
			</details>
