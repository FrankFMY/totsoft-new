	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		...
		>
			...
			<div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
				<div>
					<h3 class="text-2xl font-bold text-white mb-2">Юридическая прозрачность</h3>
					...
					<div class="space-y-3">
						<div class="flex flex-wrap gap-2 text-sm font-mono text-slate-500">
							<span class="px-3 py-1 bg-white/5 rounded">ИНН {COMPANY_INFO.inn}</span>
							<span class="px-3 py-1 bg-white/5 rounded">КПП {COMPANY_INFO.kpp}</span>
							<span class="px-3 py-1 bg-white/5 rounded" title="от {COMPANY_INFO.ogrn.date}">
								ОГРН {COMPANY_INFO.ogrn.number}
							</span>
						</div>
						<div class="flex flex-wrap gap-2 text-xs font-mono text-slate-500">
							{#each COMPANY_INFO.okved as okved (okved.code)}
								<span class="px-3 py-1 bg-white/5 rounded" title={okved.description}>
									ОКВЭД {okved.code}
								</span>
							{/each}
						</div>
