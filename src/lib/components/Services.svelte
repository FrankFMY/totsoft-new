<script lang="ts">
	import { FileText, Rocket, Settings, Users2 } from 'lucide-svelte';
...
<section id="services" class="py-24 bg-dark-800 relative">
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		<div class="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
			<div class="max-w-2xl">
				<h2 class="text-3xl md:text-5xl font-bold text-white mb-4">Что мы умеем</h2>
				<p class="text-slate-400 text-lg">
					Мы закрываем полный цикл производства ПО: от первого созвона и аналитики до многолетней
					поддержки серверов. Беремся за задачи, от которых отказываются другие.
				</p>
			</div>
			<a
				href="#contact"
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-8">
			{#each services as service (service.title)}
				...
				<div
					class="group bg-dark-900 hover:bg-dark-900/80 border border-white/5 hover:border-primary-500/30 p-8 rounded-2xl transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-primary-900/10 flex flex-col h-full"
				>
					<div
						class="w-16 h-16 bg-dark-800 rounded-xl flex items-center justify-center mb-6 group-hover:bg-primary-500/10 group-hover:text-white transition-colors border border-white/5"
					>
						<Icon class="w-8 h-8 {service.iconColor}" />
					</div>
					<h3
		...
		<div class="mt-12 text-center md:hidden">
			<a href="#contact" class="inline-flex items-center gap-2 text-primary-400 font-medium">
				Запросить прайс-лист <Settings size={16} />
			</a>
		</div>
	</div>
